...
ENTRIES = [PKG / "__main__.py", ROOT / "packaging" / "phantom_launcher.py"]
...
# NOTE: the CI steps that launched the built .exe, verified the .msi and
# started the portable build were removed on request after repeatedly hanging
# the runner. The tests that asserted their existence went with them.
#
# The protections they relied on still matter if they are ever reinstated:
# absolute exe path, WaitForExit timeout, taskkill /T on the process tree,
# and continue-on-error. See git history for a working implementation.
#
# The WMI cost controls below are NOT CI-specific - they keep the shipped app
# responsive on a real user's machine, which is why they stay.


@pytest.mark.repo_hygiene
def test_the_build_job_has_a_timeout():
    """Whatever the steps do, the job itself must be bounded."""
    import yaml
    ...
    job = data["jobs"]["build"]
    assert job.get("timeout-minutes"), "the build job can run forever"


@pytest.mark.repo_hygiene
def test_the_build_step_has_a_timeout():
    import yaml
    wf = ROOT / ".github" / "workflows" / "build.yml"
    if not wf.is_file():
        pytest.skip("workflow missing from this checkout")
    ...
    for step in data["jobs"]["build"]["steps"]:
        if "build.ps1" in str(step.get("run", "")):
            assert step.get("timeout-minutes"), (
                "build.ps1 runs the tests and PyInstaller with no time limit")
            return
    pytest.fail("no build.ps1 step found")
