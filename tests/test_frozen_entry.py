...
def _smoke_step():
    ...
    match = [s for s in steps if "EXE actually starts" in str(s.get("name", ""))]
    assert match, "the exe smoke-test step is missing"
    return match[0]


@pytest.mark.repo_hygiene
def test_ci_smoke_test_cannot_hang():
    """A hanging smoke test burns the whole job and reports nothing."""
    step = _smoke_step()
    run = step["run"]
    assert step.get("timeout-minutes"), (
        "the smoke-test step has no wall-clock ceiling")
    assert "WaitForExit" in run, "no per-command time limit"
    assert "PHANTOM_WMI_BUDGET" in run, "CI does not cap WMI cost"


@pytest.mark.repo_hygiene
def test_smoke_test_uses_an_absolute_exe_path():
    """A relative path failed to resolve in a background job and the step
    waited forever on a process that never started."""
    run = _smoke_step()["run"]
    assert "Join-Path $PWD" in run, "the exe path is relative"
    assert '$exe = ".\\dist' not in run


@pytest.mark.repo_hygiene
def test_smoke_test_kills_the_whole_process_tree():
    """A frozen build spawns PowerShell for WMI. Killing only the parent
    leaves children holding the runner open."""
    run = _smoke_step()["run"]
    assert "taskkill" in run and "/T" in run, (
        "a timed-out command's child processes are never killed")


@pytest.mark.repo_hygiene
def test_smoke_test_never_fails_the_build():
    step = _smoke_step()
    assert step.get("continue-on-error") is True, (
        "a smoke-test problem would discard an otherwise good build")


@pytest.mark.repo_hygiene
def test_every_ci_step_that_runs_the_app_is_bounded():
    """Any step invoking the built exe must have a timeout."""
    import yaml
    wf = ROOT / ".github" / "workflows" / "build.yml"
    if not wf.is_file():
        pytest.skip("workflow missing from this checkout")
    data = yaml.safe_load(wf.read_text(encoding="utf-8"))
    job = data["jobs"]["build"]
    assert job.get("timeout-minutes"), "the build job has no timeout"
    for step in job["steps"]:
        run = str(step.get("run", ""))
        if "PhantomTweeks.exe" in run and "WaitForExit" not in run:
            assert step.get("timeout-minutes"), (
                f"step {step.get('name')!r} runs the exe with no time limit")
