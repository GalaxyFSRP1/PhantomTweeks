...
def test_no_overclocking_anywhere():
    ...
    # Match on what the code CALLS, not on prose. Descriptions legitimately
    # mention voltages to say the tweak does not touch them, and grepping
    # for the word flagged those disclaimers as violations.
    tree = ast.parse(Path(perftweaks.__file__).read_text(encoding="utf-8"))
    banned_args = ("-lgc", "--lock-gpu-clocks", "-lmc", "--power-limit",
                   "-pl", "--gpu-target-temp", "setvoltage", "coreoffset",
                   "memoryoffset", "curveoptimizer")
    for node in ast.walk(tree):
        if not isinstance(node, ast.Constant) or not isinstance(node.value, str):
            continue
        text = node.value.lower().strip()
        # Only literal command fragments matter - a sentence is not a call.
        if len(text.split()) > 3:
            continue
        for bad in banned_args:
            assert text != bad and not text.startswith(bad + " "), (
                f"perftweaks passes {bad!r} - that changes clocks or voltage")
