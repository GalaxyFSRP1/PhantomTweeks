...
# ------------------------------------------------- versioned MSI download
#
# The release asset is PhantomTweeks-<version>.msi. The allowlist held only
# static filenames, so no .msi could ever be served - the download link 404'd
# at every version.

def test_versioned_msi_filenames_are_allowed():
    dl = _load_api("download")
    for name in ("PhantomTweeks-0.1.4.msi", "PhantomTweeks-0.1.5.msi",
                 "PhantomTweeks-1.0.0.msi", "PhantomTweeks-2.0.0.1.msi"):
        assert dl.is_allowed(name), f"{name} would 404"


def test_lookalike_and_traversal_names_are_rejected():
    """The allowlist must stay a allowlist, not become a wildcard."""
    dl = _load_api("download")
    for name in ("evil.msi", "PhantomTweeks-abc.msi", "../../etc/passwd",
                 "PhantomTweeks-0.1.4.msi/../x", "..\\\\windows\\\\system32",
                 "PhantomTweeks-.msi", "notPhantomTweeks-1.0.msi"):
        assert not dl.is_allowed(name), f"{name} should be refused"


def test_msi_alias_resolves_from_the_release():
    """The exact filename is only knowable from the release itself."""
    dl = _load_api("download")
    release = {"tag_name": "v0.1.5", "assets": [
        {"name": "PhantomTweeks-Setup.exe", "id": 1, "size": 10},
        {"name": "PhantomTweeks-0.1.5.msi", "id": 2, "size": 20},
    ]}
    assert dl.resolve_name("msi", release) == "PhantomTweeks-0.1.5.msi"
    assert dl.resolve_name("setup", release) == "PhantomTweeks-Setup.exe"
    assert dl.resolve_name("nonsense", release) is None


def test_msi_resolves_to_the_right_asset_id():
    dl = _load_api("download")
    release = {"tag_name": "v0.1.5", "assets": [
        {"name": "PhantomTweeks-Setup.exe", "id": 1, "size": 10},
        {"name": "PhantomTweeks-0.1.5.msi", "id": 2, "size": 99},
    ]}
    ...
    dl._gh = lambda u, t, a: Resp()
    asset_id, size, tag = dl.resolve_asset("msi", "token", "owner/repo")
    assert asset_id == 2, "the MSI was not found in the release"
    assert size == 99
    assert tag == "v0.1.5"


def test_a_release_without_an_msi_reports_missing_not_wrong_file():
    dl = _load_api("download")
    release = {"tag_name": "v0.1.5", "assets": [
        {"name": "PhantomTweeks-Setup.exe", "id": 1, "size": 10}]}
    assert dl.resolve_name("msi", release) is None
