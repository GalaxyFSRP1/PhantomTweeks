...
$Work = Join-Path $Root "packaging\_work"
...
$Launcher = Join-Path $Root "packaging\phantom_launcher.py"
if (-not (Test-Path $Launcher)) {
    Fail "packaging\phantom_launcher.py is missing. The frozen build needs it: " +
...
& $VPy -m PyInstaller (Join-Path $Root "packaging\PhantomTweeks.spec") `
...
Remove-Item Env:\PHANTOM_PORTABLE -ErrorAction SilentlyContinue
...
& $VPy -m PyInstaller (Join-Path $Root "packaging\Updater.spec") `
    --distpath $Dist --workpath (Join-Path $Work "updater") --noconfirm --clean
if ($LASTEXITCODE -ne 0) { Fail "PyInstaller failed for Update.exe." }
if (-not (Test-Path (Join-Path $Dist "Update.exe"))) {
    Fail "PyInstaller reported success but dist\Update.exe is missing."
}
Write-Ok "dist\Update.exe"
...
if (-not $SkipInstaller) {
    ...
    if ($iscc) {
        & $iscc (Join-Path $Root "packaging\installer.iss") /Q
...
if (-not $SkipMsi) {
    ...
    if (-not $wix) {
        ...
        $wixArgs = @(
            "build", (Join-Path $Root "packaging\PhantomTweeks.wxs"),
            "-arch", "x64",
