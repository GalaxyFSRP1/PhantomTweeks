...
# 2. Commit
git add -A && git commit -m "Release 0.2.6"
...
`https://github.com/GalaxyFSRP1/PhantomTweeks/releases/download/v0.2.6/PhantomTweeks-Setup.exe`
...
| `PhantomTweeks-0.2.6.msi` | Managed deployment — Group Policy, Intune, SCCM, or `msiexec`. |
...
```powershell
msiexec /i PhantomTweeks-0.2.6.msi                  # normal install
msiexec /i PhantomTweeks-0.2.6.msi /quiet /norestart  # silent
msiexec /i PhantomTweeks-0.2.6.msi INSTALLDESKTOPSHORTCUT=0   # no desktop icon
msiexec /x PhantomTweeks-0.2.6.msi                  # uninstall
```
