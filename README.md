...
- **Fixed the MSI download** — the versioned filename could never match the
  static allowlist, so `.msi` downloads 404'd at every version.
- **Tweak Encyclopedia: 253 tweaks** — 73 safe,
  102 situational, 37 placebo, 41 harmful,
  each with the reasoning behind its verdict and searchable by registry value.

### What's in 0.1.4

- **Tweak Encyclopedia** — 29 popular gaming tweaks with a verdict
  each: 7 safe, 5 situational, 8 placebo,
  9 harmful. Searchable by registry value. Documents the dangerous
  ones rather than hiding them, because a user who cannot find an honest answer
  here will find a .reg file elsewhere with no warning attached.
