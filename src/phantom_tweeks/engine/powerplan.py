...
def find_phantom_plan() -> Optional[str]:
    """Find our plan, including any left by an earlier broken version."""
...
def create_plan(activate: bool = True) -> tuple[bool, list[str], list[ChangeRecord]]:
    ...
    if existing:
        ...
        guid = m.group(1).lower()
        rename = _powercfg("/changename", guid, PHANTOM_PLAN_NAME,
                           PHANTOM_PLAN_DESC)
