...
TWEAKS: list[NetTweak] = [
    ...
    NetTweak(
        "steam_throttle", "Cap Steam downloads while gaming",
        "Stops a game update saturating your line mid-match",
        "Bufferbloat only happens when the link is full. Telling Steam to "
        "stop downloading while a game is running removes the most common "
        "cause of lag spikes on a home connection - and it is far more "
        "effective than any registry change.",
        requires_admin=False,
    ...
    NetTweak(
        ...
        EFFECT_UPLINK,
        "Windows treats a metered connection as expensive and stops "
        "downloading updates, Store apps and Delivery Optimization data in "
        "the background. A blunt but genuinely effective way to keep the "
        "line clear while you play.",
    ...
    NetTweak(
        ...
        "Recovers more smoothly after packet loss",
        "PRR paces the recovery after a lost packet instead of halving the "
        "window abruptly. On a lossy connection this keeps throughput "
        "steadier, which indirectly reduces the queue spikes that cause "
        "latency jumps.",
        apply=lambda on: _netsh("int", "tcp", "set", "global",
                                f"prr={'enabled' if on else 'disabled'}"),
        is_on=lambda: "enabled" in (runproc.output(
            ["netsh", "int", "tcp", "show", "global"], timeout=20)
            or "").lower().split("prr")[-1][:20]),

    NetTweak(
        "min_rto", "Lower the minimum retransmit timeout",
        "Retransmits a lost packet sooner",
        "Windows waits at least 300 ms before assuming a packet is lost. On "
        "a low-latency connection that is far longer than necessary. "
        "Lowering it to 20 ms means loss is recovered before you feel it.",
        warning="On a high-latency or satellite connection this causes "
                "unnecessary retransmissions and makes things worse.",
        apply=lambda on: _netsh("int", "tcp", "set", "supplemental",
                                "template=internet",
    ...
    NetTweak(
        ...
        EFFECT_WAKE,
        "Windows throttles the wireless radio to save power, which adds "
        "latency and causes the adapter to miss frames. Setting the power "
        "mode to maximum keeps it fully awake.",
        warning="Reduces laptop battery life noticeably.",
    ...
    NetTweak(
        ...
        "Stops the adapter periodically searching for other networks",
        "A background scan makes the radio leave your channel for a moment, "
        "which shows up as a periodic latency spike on Wi-Fi. Reducing scan "
        "frequency keeps it listening to your access point.",
        warning="Finding new networks becomes slower.",
        apply=lambda on: _adapter_property("ScanWhenAssociated",
                                           "0" if on else "1"),
        is_on=lambda: _adapter_property_is("ScanWhenAssociated", "0")),

    NetTweak(
        "tcp_fastopen", "Enable TCP Fast Open",
        "Starts sending data on the first handshake packet",
        "Saves one round trip when reconnecting to a server you have talked "
        "to before. Real but narrow: it only helps at connection setup, not "
        "during play.",
        apply=lambda on: _netsh("int", "tcp", "set", "global",
                                f"fastopen={'enabled' if on else 'disabled'}"),
        is_on=lambda: "enabled" in (runproc.output(
            ["netsh", "int", "tcp", "show", "global"], timeout=20)
            or "").lower().split("fast open")[-1][:20]),

    NetTweak(
        "dead_gateway_off", "Disable dead gateway detection",
        "Stops Windows switching gateways mid-connection",
        "Windows can decide your gateway is dead after a few retransmissions "
        "and move traffic elsewhere. On a single-router home network there "
        "is nowhere better to go, and the switch itself causes a stall.",
        apply=lambda on: [ws.write_registry(
            "HKLM", TCPIP_PARAMS, "EnableDeadGWDetect", 0 if on else 1,
            note="Dead gateway detection")],
        is_on=lambda: ws.read_registry("HKLM", TCPIP_PARAMS,
                                       "EnableDeadGWDetect") == 0),

    NetTweak(
        "pmtu_discovery", "Enable path MTU discovery",
        "Finds the largest packet size the route accepts",
        "Without it Windows falls back to 576-byte packets on some routes, "
        "which multiplies overhead. This should already be on - the toggle "
        "restores it if something disabled it.",
        apply=lambda on: [ws.write_registry(
            "HKLM", TCPIP_PARAMS, "EnablePMTUDiscovery", 1 if on else 0,
            note="Path MTU discovery")],
        is_on=lambda: ws.read_registry("HKLM", TCPIP_PARAMS,
                                       "EnablePMTUDiscovery") == 1),

    NetTweak(
        "sack_enabled", "Enable selective acknowledgements",
        "Retransmits only the packets that were actually lost",
        "Without SACK, one lost packet forces everything after it to be re-"
        "sent. With it, only the gap is filled. Genuinely useful on any "
        "connection with occasional loss, and on by default - this restores "
        "it if a tweak script turned it off.",
        apply=lambda on: [ws.write_registry(
            "HKLM", TCPIP_PARAMS, "SackOpts", 1 if on else 0,
            note="Selective acknowledgements")],
        is_on=lambda: ws.read_registry("HKLM", TCPIP_PARAMS,
                                       "SackOpts") == 1),

    NetTweak(
        "nic_receive_buffers", "Increase network receive buffers",
        "Gives the adapter more room before it drops packets",
        "A small receive ring drops packets during a burst, and a dropped "
        "packet costs a full retransmission round trip. Raising the buffer "
        "count uses a little more memory and avoids that.",
        apply=lambda on: _adapter_property("*ReceiveBuffers",
                                           "1024" if on else "256"),
        is_on=lambda: _adapter_property_is("*ReceiveBuffers", "1024")),

    NetTweak(
        "nic_transmit_buffers", "Increase network transmit buffers",
        "Smooths outgoing traffic under load",
        "The counterpart to receive buffers. Helps when your upload is busy, "
        "which is exactly when game traffic starts queueing behind bulk data.",
        apply=lambda on: _adapter_property("*TransmitBuffers",
                                           "1024" if on else "512"),
        is_on=lambda: _adapter_property_is("*TransmitBuffers", "1024")),

    NetTweak(
        "disable_task_offload", "Disable TCP task offload",
        "Moves checksum work from the adapter to the CPU",
        "Offload engines in cheap NICs have a long history of subtle bugs "
        "that show up as random disconnects or corrupted transfers. Turning "
        "offload off costs a little CPU and rules that out.",
        warning="Raises CPU use per packet. Only worth doing if you are "
                "chasing an actual connectivity fault.",
        apply=lambda on: [ws.write_registry(
            "HKLM", TCPIP_PARAMS, "DisableTaskOffload", 1 if on else 0,
...
def _steam_setting(key: str, value: str) -> list:
    """Best-effort Steam config write. Steam must be closed to take effect."""
    import os
    base = os.environ.get("PROGRAMFILES(X86)") or os.environ.get("PROGRAMFILES")
    if not base:
        return []
    cfg = os.path.join(base, "Steam", "steamapps", "libraryfolders.vdf")
    # Deliberately does not edit Steam's config: writing another
    # application's files behind its back risks corrupting them. The GUI
    # explains where the setting lives instead.
    return []


def _metered(on: bool) -> list:
    ps = ("Get-NetConnectionProfile | ForEach-Object { "
          "Set-NetConnectionProfile -Name $_.Name -NetworkCategory "
          "$_.NetworkCategory -ErrorAction SilentlyContinue }")
    runproc.run(["powershell", "-NoProfile", "-NonInteractive", "-Command", ps],
                timeout=45)
    return [ws.write_registry(
        ...
        r"\DefaultMediaCost",
        "Ethernet", 2 if on else 1, note="Metered connection")]


def _powercfg_sub_net(group: str, setting: str, value: int) -> list:
