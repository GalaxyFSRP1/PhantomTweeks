...
def confirm_dialog(parent, title: str, message: str, danger: bool = False,
    ...
    """Multi-step confirmation. ``steps=2`` for anything that weakens safety.

    The dialog sizes itself to its content and never lets the buttons leave
    the screen. An earlier version used a fixed 540x330 window with a plain
    label, so a long message - a list of twenty tweaks, say - pushed the
    Confirm button below the bottom edge where it could not be clicked.

    Three things prevent that now:
      * buttons and checkboxes are packed against the bottom FIRST, so they
        reserve their height before the message gets any;
      * the message lives in a scrolling text area, so it can be any length;
      * the window is measured against the actual screen and capped, then
    ...
    # --- bottom-anchored controls first, so they always keep their space ---
    btns = ttk.Frame(frame, style="Panel.TFrame")
    btns.pack(side="bottom", fill="x", pady=(16, 0))

    acks = []
    ack_box = ttk.Frame(frame, style="Panel.TFrame")
    ack_box.pack(side="bottom", fill="x", pady=(12, 0))
    ack1 = tk.BooleanVar()
    ttk.Checkbutton(ack_box, text="I understand what this changes.",
    ...
    # --- the message takes whatever height is left, and scrolls ---
    lines = message.count("\n") + 1
    body = tk.Text(frame, wrap="word", height=min(18, max(4, lines + 1)),
    ...
    body.pack(side="left", fill="both", expand=True, pady=(12, 0))
    ...
    # --- size to content, but never larger than the screen ---
    win.update_idletasks()
    want_w = max(520, min(760, win.winfo_reqwidth()))
    want_h = min(win.winfo_reqheight(), int(win.winfo_screenheight() * 0.85))
    want_h = max(240, want_h)
    px, py = parent.winfo_rootx(), parent.winfo_rooty()
    pw, ph = parent.winfo_width(), parent.winfo_height()
    x = max(0, px + (pw - want_w) // 2)
    y = max(0, py + (ph - want_h) // 3)
