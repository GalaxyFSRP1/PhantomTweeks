...
def apply(root: tk.Misc) -> ttk.Style:
    ...
    style.map("Accent.TButton",
              ...
              # Without this the dark-on-accent text stayed at full contrast
              # while the fill greyed out, so a disabled button looked broken.
              foreground=[("disabled", c["muted"])])
    ...
    # "Liquid glass" surfaces. Tk has no real blur or alpha compositing, so
    # the effect is built from layered flat tones: a slightly lighter panel,
    # a bright hairline along the top edge to read as a specular highlight,
    # and a darker base. Faking translucency with stippled images looked
    # muddy at small sizes, so this is deliberately clean rather than blurry.
    style.configure("Glass.TFrame", background=c["glass"], relief="flat",
                    borderwidth=0)
    style.configure("GlassEdge.TFrame", background=c["glass_edge"],
                    relief="flat", borderwidth=0)
    style.configure("Glass.TLabel", background=c["glass"], foreground=c["text"])
    style.configure("GlassMuted.TLabel", background=c["glass"],
                    foreground=c["muted"], font=FONT_SMALL)
    style.configure("GlassAccent.TLabel", background=c["glass"],
                    foreground=c["accent"], font=FONT_BOLD)
    style.configure("GlassHuge.TLabel", background=c["glass"],
                    foreground=c["accent"], font=FONT_HUGE)
    style.configure("Glass.TCheckbutton", background=c["glass"],
                    foreground=c["text"], indicatorbackground=c["bg"],
                    indicatorforeground=c["accent"], padding=(2, 3))
    style.map("Glass.TCheckbutton",
              background=[("active", c["glass"])],
    ...
    style.configure("Tab.TButton", background=c["bg"], foreground=c["muted"],
                    padding=(20, 11), font=FONT, relief="flat", borderwidth=0)
    style.map("Tab.TButton",
              background=[("active", c["glass"])],
              foreground=[("active", c["text"])])
    style.configure("TabActive.TButton", background=c["glass"],
                    foreground=c["accent"], padding=(20, 11),
                    font=FONT_BOLD, relief="flat", borderwidth=0)
    style.map("TabActive.TButton", background=[("active", c["glass"])])
