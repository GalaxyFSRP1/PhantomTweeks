...
from ..engine import maintenance, history, tweakdb, catalog
...
PAGE_SECTION = {
    ...
    "dns": "dashboard", "games": "dashboard", "background": "dashboard",
    "health": "dashboard", "trends": "dashboard",
...
class PhantomWindow(tk.Tk):
    ...
    def _build_pages(self) -> None:
        ...
        self._build_startup()
        self._build_simple("drivers", "Driver Center",
                           "Phantom Tweeks never downloads or installs drivers "
                           "automatically. Use the official vendor link.",
                           self._drivers_text)
    ...
    def _build_dashboard(self) -> None:
        ...
        self.score_chart = BarChart(left, height=210)
        self.score_chart.set_empty_text(
            ...
            "rather than guessed.")
        # Pack the button against the bottom BEFORE the chart. A chart with
        # expand=True otherwise starves it and it renders as a clipped sliver.
        ttk.Button(left, text="Calculate Score", style="Accent.TButton",
                   command=self._calc_score).pack(side="bottom", anchor="w",
                                                  pady=(8, 0))
        self.score_chart.pack(side="top", fill="both", expand=True, pady=(4, 0))

        right = Card(mid, "GAME DETECTION")
        right.grid(row=0, column=1, sticky="nsew")
        self.game_text = theme.make_text(right, height=11)
        self.game_text.pack(fill="both", expand=True, pady=(6, 8))
        self._write(self.game_text, "Scanning for games…")
        self.mode_btn = ttk.Button(right, text="Enable Gaming Mode",
                                   style="Accent.TButton", command=self._toggle_mode)
        self.mode_btn.pack(fill="x")
        ttk.Label(right, text="Gaming Mode monitors and protects. It never closes "
    ...
    def _build_settings(self) -> None:
        ...
        groups = {
            ...
            "Diagnostics": [("expert_mode", "⚠ Expert Mode (detailed technical data)", True)],
        }
