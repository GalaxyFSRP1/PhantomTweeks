...
class PhantomWindow(tk.Tk):
    ...
    def _build_power(self) -> None:
        ...
        top = ttk.Frame(page)
        top.pack(fill="x", pady=(0, 10))
        ttk.Button(top, text="Create Gaming plan", style="Accent.TButton",
                   command=lambda: self._create_power_plan("gaming")
                   ).pack(side="left")
        ttk.Button(top, text="Create MAXIMUM plan", style="Chip.TButton",
                   command=lambda: self._create_power_plan("maximum")
    ...
    def _create_power_plan(self, profile: str = "gaming") -> None:
        chosen = powerplan.PROFILES.get(profile) or powerplan.PROFILES["gaming"]
        laptop = powerplan.is_laptop()

        body = [chosen.description, "",
                f"This creates a NEW power plan with "
                f"{chosen.setting_count} tuned settings. Your existing plans "
                "are not modified, and removing it returns Windows to what "
                "was active before.", ""]
        body.append("What it changes:")
        for item in chosen.settings:
            body.append(f"  - {item.label}")
        if laptop and chosen.laptop_warning:
            body += ["", "LAPTOP DETECTED", "", chosen.laptop_warning]

        if not confirm_dialog(self, f"Create '{chosen.name}'?",
                              "\n".join(body),
                              danger=bool(laptop and chosen.laptop_warning),
                              steps=2 if (laptop and chosen.laptop_warning)
                              else 1):
            return

        def done(result):
            ok, msgs, _changes = result
            self._write(self.power_text, "\n".join(msgs))
            Toast(self, msgs[0] if msgs else "Done",
                  "success" if ok else "warning")
            self._refresh_power()

        self._run_async(lambda: powerplan.create_plan(True, profile), done,
                        f"Creating {chosen.name}...")
