...
def cmd_power(app: PhantomApp, args) -> int:
    ...
    if args.create:
        profile = getattr(args, "profile", "gaming")
        ok, msgs, _ = app.create_power_plan(
            activate=not args.no_activate, profile=profile)
        for m in msgs:
            print(" ", m)
...
def cmd_tweaks(app: PhantomApp, args) -> int:
    """The tweak encyclopedia: look up any popular Windows gaming tweak."""
    from ..engine import tweakdb

    if args.search:
        hits = tweakdb.search(args.search)
        _h(f"TWEAK LOOKUP: {args.search}")
        if not hits:
            print(f"Nothing matching {args.search!r}.")
            print()
            print("This covers the tweaks that circulate widely. If you found")
            print("something not listed, treat it with suspicion until you can")
            print("explain what it actually changes.")
            return 0
        for t in hits:
            print(t.render())
            print()
    ...
    if args.verdict:
        ...
        _h(f"{verdict} TWEAKS ({len(items)})")
        for t in items:
            print(t.render())
            print()
        return 0

    print(tweakdb.report())
    return 0


def cmd_perf(app: PhantomApp, args) -> int:
    """Consolidated performance view: CPU, GPU, memory, every drive."""
    from ..engine import perfview
    import time as _time
...
def cmd_restorepoint(app: PhantomApp, args) -> int:
    """Windows System Restore points - an extra layer beyond our journal."""
    from ..engine import restorepoint

    if args.create:
        _h("CREATE RESTORE POINT")
        ok, message = restorepoint.create(args.description)
        print(message)
        return 0 if ok else 1

    _h("SYSTEM RESTORE")
    print(restorepoint.status().render())
    points = restorepoint.list_points()
    if points:
        print()
...
def cmd_report(app: PhantomApp, args) -> int:
    """Export a full diagnostic report."""
    from ..engine import reports

    if not args.output:
        print(reports.build().render(args.format))
        return 0
    ok, message = reports.save(args.output, args.format)
    print(message)
    return 0 if ok else 1


def cmd_nettweaks(app: PhantomApp, args) -> int:
    """Network tweaks - simple on/off toggles."""
    from ..engine import nettweaks

    if args.on or args.off:
        target = args.on or args.off
        enable = bool(args.on)
        if target == "all":
            picked = ([t for t in nettweaks.TWEAKS if not t.warning]
                      if enable else list(nettweaks.TWEAKS))
            if enable:
                print("Turning on the tweaks with no caution note.")
                print("Tweaks that can backfire are skipped; enable those by "
                      "name.")
        else:
            if target not in nettweaks.BY_ID:
                print(f"Unknown tweak: {target}")
                print("Available: " + ", ".join(sorted(nettweaks.BY_ID)))
                return 1
            picked = [nettweaks.BY_ID[target]]
        _h("NETWORK TWEAKS")
        failures = 0
        for t in picked:
            ok, message, _ = nettweaks.set_tweak(t.id, enable)
            print(("  ok   " if ok else "  fail ") + message)
            failures += 0 if ok else 1
        return 0 if not failures else 1

    print(nettweaks.summary())
    print()
    print("Turn one on:   run.py nettweaks --on nagle")
    print("Turn all on:   run.py nettweaks --on all")
    print("Revert all:    run.py nettweaks --off all")
    return 0


def cmd_perftweaks(app: PhantomApp, args) -> int:
    """Latency and performance tweaks as simple toggles."""
    from ..engine import perftweaks

    if args.on or args.off:
        target = args.on or args.off
        enable = bool(args.on)
        if target == "all":
            picked = ([t for t in perftweaks.TWEAKS if not t.warning]
                      if enable else list(perftweaks.TWEAKS))
            if enable:
                print("Applying the tweaks with no caution note. Tweaks that")
                print("can backfire are skipped; enable those by name.")
        else:
            if target not in perftweaks.BY_ID:
                print(f"Unknown tweak: {target}")
                print("Available: " + ", ".join(sorted(perftweaks.BY_ID)))
                return 1
            picked = [perftweaks.BY_ID[target]]
        _h("LATENCY TWEAKS")
        bad = 0
        for t in picked:
            ok, message, _ = perftweaks.set_tweak(t.id, enable)
            print(("  ok   " if ok else "  fail ") + message)
            bad += 0 if ok else 1
        return 0 if not bad else 1

    print(perftweaks.summary())
    print("Turn one on:  run.py perftweaks --on mouse_accel")
    print("Turn all on:  run.py perftweaks --on all")
    print("Revert all:   run.py perftweaks --off all")
    return 0


def cmd_nvidia(app: PhantomApp, args) -> int:
    """NVIDIA clocks, power limits and throttle reasons."""
    from ..hardware import nvidia

    if args.why:
        _h("WHAT IS LIMITING YOUR CLOCKS")
        findings = nvidia.diagnose()
        if not findings:
            print("Nothing is limiting your clocks - no power cap, thermal")
            print("limit or PCIe downgrade was reported.")
            return 0
        for f in findings:
            print(f"  - {f}")
            print()
...
def cmd_registry(app: PhantomApp, args) -> int:
    """Inspect the registry values Phantom Tweeks manages."""
    from ..engine import registry as regview

    if args.search:
        hits = regview.find(args.search)
        _h(f"REGISTRY: {args.search}")
        if not hits:
            print(f"No managed value matches {args.search!r}.")
            return 0
        for m in hits:
            print(m.render())
            print()
    ...
    if args.changed:
        ...
        _h("CHANGED REGISTRY VALUES")
        if not rows:
            print("Nothing differs from the Windows default.")
            return 0
    ...
    print(regview.report())
    return 0


def cmd_bufferbloat(app: PhantomApp, args) -> int:
    """Measure latency while the connection is busy."""
    from ..network import bufferbloat

    _h("BUFFERBLOAT")
    print("Measuring. This briefly saturates your connection.")
    print()
    result = bufferbloat.run(seconds_per_phase=args.seconds,
...
def cmd_cpu(app: PhantomApp, args) -> int:
    """CPU detail and vendor-specific guidance."""
    from ..hardware import cpuvendor
    print(cpuvendor.report())
    return 0


def cmd_driverupdate(app: PhantomApp, args) -> int:
    """Check the installed GPU driver against the published version."""
    from ..engine import driverupdate
    print(driverupdate.report())
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--profile", choices=["gaming", "maximum"],
                   default="gaming",
                   help="which plan tier to create (default gaming)")
    ...
    s = sub.add_parser("restore-point",
                       help="Windows System Restore points")
    s.add_argument("--create", action="store_true",
                   help="create a restore point (needs admin)")
    s.add_argument("--description", default="Before Phantom Tweeks changes")
    s.set_defaults(fn=cmd_restorepoint)

    s = sub.add_parser("report", help="export a full diagnostic report")
    s.add_argument("-o", "--output", help="file to write (format from extension)")
    s.add_argument("--format", default="txt",
                   choices=["html", "json", "csv", "md", "txt"])
    s.set_defaults(fn=cmd_report)

    s = sub.add_parser("bufferbloat",
                       help="measure latency while the connection is busy")
    s.add_argument("--seconds", type=float, default=6.0,
                   help="seconds per phase (default 6)")
    s.set_defaults(fn=cmd_bufferbloat)

    s = sub.add_parser("registry", help="inspect managed registry values")
    s.add_argument("--search", metavar="TERM", help="find a value")
    s.add_argument("--changed", action="store_true",
                   help="show only values that differ from the default")
    s.set_defaults(fn=cmd_registry)

    s = sub.add_parser("perftweaks",
                       help="latency and performance tweaks as toggles")
    s.add_argument("--on", metavar="ID", help="enable a tweak, or 'all'")
    s.add_argument("--off", metavar="ID", help="revert a tweak, or 'all'")
    s.set_defaults(fn=cmd_perftweaks)
    ...
    s = sub.add_parser("nvidia", help="NVIDIA clocks and throttle reasons")
    s.add_argument("--why", action="store_true",
                   help="explain what is limiting your clocks")
    s.set_defaults(fn=cmd_nvidia)

    s = sub.add_parser("nettweaks", help="network tweaks as simple toggles")
    s.add_argument("--on", metavar="ID", help="enable a tweak, or 'all'")
    s.add_argument("--off", metavar="ID", help="revert a tweak, or 'all'")
    s.set_defaults(fn=cmd_nettweaks)

    s = sub.add_parser("perf", help="all performance data including every drive")
    s.add_argument("--watch", action="store_true", help="refresh continuously")
    s.add_argument("--interval", type=float, default=5.0,
                   help="seconds between refreshes when watching")
    s.set_defaults(fn=cmd_perf)

    s = sub.add_parser("tweaks", help="encyclopedia of popular gaming tweaks")
    s.add_argument("--search", metavar="TERM",
                   help="look up a tweak by name or registry value")
    s.add_argument("--verdict", choices=["safe", "situational", "placebo",
                                         "harmful"],
                   help="list only tweaks with this verdict")
