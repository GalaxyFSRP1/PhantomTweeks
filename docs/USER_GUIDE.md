...
Phantom Tweeks checks for a newer version **once a day**, quietly, in the
background. If one exists a banner appears at the top of the window with three
choices: view it, skip that version permanently, or dismiss (which snoozes
reminders for a week).

**It will never interrupt you mid-game.** Update checks are suppressed while a
game is detected.
...
python run.py update --download   # download and verify
```
...
| --- | --- |
| Use HTTPS with certificate verification | Accept a plain-HTTP URL |
| Verify the SHA-256 published with the release | Install a file whose checksum does not match |
| Save the installer to your data folder | Run the installer for you |
| Tell you exactly where it saved it | Change anything without you asking |

If a download does not match its published checksum it is **deleted**, not
offered anyway. There is no "continue regardless" option, because a modified
installer is the single most dangerous thing this app could hand you.

The binaries are not code-signed yet, so SmartScreen will warn on first run.
Until that changes, the SHA-256 check is what proves the file is genuine.

Turn the automatic check off in **Settings → Updates** if you prefer to check
manually.
